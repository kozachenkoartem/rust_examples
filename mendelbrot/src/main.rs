...
fn main(){
    ...
    if args.len() != 5 {
        writeln!(std::io::stderr(), "Пример: {} mendel.png 500x400 -2,1 1,-1", args[0]).unwrap();
        std::process::exit(1);
    }

    let bounds = parse_pair(&args[2], 'x').expect("Ошибка при разборе размеров изображения");
    let upper_left = parse_complex(&args[3]).expect("Ошибка при разборе левого верхого угла");
    let lower_right = parse_complex(&args[4]).expect("Ошибка при разборе правого угла");

    let mut pixels = vec![0; bounds.0 * bounds.1];
    render(&mut pixels, bounds, upper_left, lower_right);
    write_image(&args[1], &pixels, bounds).expect("Ошибка при записи в файл");
